...
## Update — SDK rollback 57→54 + ledger overdue (Jun 2026, session 6)
- Rolled Expo SDK 57 back to **54** at user request: App/Play Store Expo Go is frozen at SDK 54, so an SDK 57 project could not be opened via QR. Reverted package.json (expo 54.0.37, RN 0.81.5), app.json (re-added newArchEnabled + android.edgeToEdgeEnabled, removed rnvi/auto plugins), use-icon-fonts.ts, and all 29 icon imports @react-native-vector-icons/ionicons → @expo/vector-icons. expo-doctor 18/18; manifest runtimeVersion exposdk:54.0.0; web + android bundles build. All post-upgrade features (daybook receipt-counting, date nav) preserved.
- **Ledger overdue**: a sale (source!=invoice) or invoice with NO money receipt linked = overdue. Backend ledger sets ev.overdue + summary.overdue_total/overdue_count. Frontend shows red OVERDUE badge + hint per timeline row and an "Overdue (no receipt): ₹X · N items" strip on the balance card (gated on overdue_count>0). Verified by testing_agent (backend pytest 3/3 + UI, iteration_26).

## Update — Daybook online collection fix (Jun 2026, session 7)
- Bug: online payments not showing in Daybook. Root cause: after the "count only when receipted" rule, a counted sale/invoice took its cash/online split from its OWN stored split, so an ONLINE money receipt raised against a cash-punched sale (online_amount=0) never appeared as online.
- Fix: for a counted sale/invoice, cash/online now come from the LINKED money receipts (actual money collected). inv_receipts_by_src & receipts_by_src carry cash_amount/online_amount; inv_total = inv_cash+inv_online. No double count (linked receipts excluded from standalone).
- Added explicit "Cash collected" (grand_total.cash) and "Online collected" (grand_total.online) tiles to the Daybook breakdown grid.
- Verified by testing_agent iteration_27: backend pytest 5/5 + UI tiles + preview not blank. (Preview blank report was a stale client view; login renders fine.)
